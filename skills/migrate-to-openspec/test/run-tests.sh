...
#   ./run-tests.sh --suite preflight   # run a single suite (preflight|inventory|translator|verifier|execution|idempotence|templates|hook|audit-config)
#
...
# ---------------------------------------------------------------------------
# Audit-config translation
# ---------------------------------------------------------------------------

assert_audit_config_translated() {
  local dir
  dir="$(setup_fixture audit-config-translated)"

  if ! run_migrate "$dir" run --auto-accept >/dev/null 2>&1; then
    echo "full run failed; cannot assert audit config translation"
    return 1
  fi

  local out="$dir/openspec/.audit-config.json"
  if [[ ! -f "$out" ]]; then
    echo "expected translated audit config at $out"
    return 1
  fi
...
# Modules: every spec entry must be a slug (no .md, no path), and only known
# capabilities should remain. The fixture lists feature-a.md, feature-b-cli.md,
# feature-c-mixed.md (real), feature-removed.md (orphan, must be dropped),
# and specs/feature-c-mixed.md (path-prefixed, must be normalized).
modules = {m["name"]: m for m in cfg.get("modules", [])}
core = modules.get("core", {})
cli = modules.get("cli", {})

if not core or not cli:
    errors.append(f"expected modules 'core' and 'cli'; got {list(modules)}")

core_specs = sorted(core.get("specs", []))
# Without --change flag, feature-d-unbuilt is a regular accepted cap and stays in the specs array.
if core_specs != ["feature-a", "feature-c-mixed", "feature-d-unbuilt"]:
    errors.append(f"core.specs = {core_specs}, expected ['feature-a', 'feature-c-mixed', 'feature-d-unbuilt']")

cli_specs = sorted(cli.get("specs", []))
if cli_specs != ["feature-b-cli"]:
    errors.append(f"cli.specs = {cli_specs}, expected ['feature-b-cli'] (feature-removed should drop)")

# mapping_cache must be reset.
if cfg.get("mapping_cache") != {}:
    errors.append(f"mapping_cache = {cfg.get('mapping_cache')}, expected {{}}")

# pitfalls preserved verbatim.
pitfalls = cfg.get("pitfalls", [])
if len(pitfalls) != 2 or not all("Fixture pitfall" in p for p in pitfalls):
    errors.append(f"pitfalls not preserved verbatim: {pitfalls}")

# extensions/excludes preserved.
if cfg.get("extensions") != ["go", "ts"]:
    errors.append(f"extensions changed: {cfg.get('extensions')}")
if cfg.get("excludes") != ["**/node_modules/**", "**/vendor/**"]:
    errors.append(f"excludes changed: {cfg.get('excludes')}")

# test_suites preserved.
ts = cfg.get("test_suites", [])
if ts != [{"dir": "src", "command": "go test ./..."}]:
    errors.append(f"test_suites changed: {ts}")

# version preserved.
if cfg.get("version") != 1:
    errors.append(f"version changed: {cfg.get('version')}")
...
PY
  ...
  # Source must be gone (cleaned up by spec-dir removal).
  if [[ -f "$dir/specs/.audit-config.json" ]]; then
    echo "expected source $dir/specs/.audit-config.json to be removed"
    return 1
  fi
}

assert_audit_config_missing_is_noop() {
  local dir
  dir="$(setup_fixture audit-config-noop)"

  # Remove the fixture's audit config to exercise the no-op path.
  rm -f "$dir/specs/.audit-config.json"
  (cd "$dir" && git add -A && git commit -q -m "remove audit config") >/dev/null

  if ! run_migrate "$dir" run --auto-accept >/dev/null 2>&1; then
    echo "full run failed on a project without audit config"
    return 1
  fi

  if [[ -f "$dir/openspec/.audit-config.json" ]]; then
    echo "openspec/.audit-config.json should not exist when no source config was present"
    return 1
  fi
}

# ---------------------------------------------------------------------------
# Change-candidate routing (--change flag)
# ---------------------------------------------------------------------------

assert_change_candidate_creates_change_folder() {
  local dir
  dir="$(setup_fixture change-candidate-folder)"

  local out rc
  out=$(run_migrate "$dir" run --auto-accept --change feature-d-unbuilt 2>&1)
  rc=$?
  if [[ $rc -ne 0 ]]; then
    echo "full run exited non-zero (rc=$rc): $out"
    return 1
  ...
  local fail=0
  local change_dir="$dir/openspec/changes/v1.0-feature-d-unbuilt"

  for f in proposal.md tasks.md specs/feature-d-unbuilt/spec.md; do
    if [[ ! -f "$change_dir/$f" ]]; then
      echo "missing $change_dir/$f"
      fail=1
    fi
  done

  # Delta must start with `## ADDED Requirements` header.
  local delta="$change_dir/specs/feature-d-unbuilt/spec.md"
  if [[ -f "$delta" ]]; then
    local first
    first=$(grep -m1 -E '^##' "$delta")
    if [[ "$first" != "## ADDED Requirements" ]]; then
      echo "expected delta first ## header to be '## ADDED Requirements'; got '$first'"
      fail=1
    fi
    if grep -q "^# Capability:" "$delta" || grep -q "^## Purpose" "$delta"; then
      echo "delta must not contain '# Capability:' or '## Purpose' headers"
      fail=1
    fi
    # Each requirement should still have at least one Scenario.
    local req_count scenario_count
    req_count=$(grep -cE '^### Requirement:' "$delta")
    scenario_count=$(grep -cE '^#### Scenario:' "$delta")
    if [[ $scenario_count -lt $req_count ]]; then
      echo "delta has $req_count requirements but only $scenario_count scenarios"
      fail=1
    fi
  fi

  # tasks.md should contain at least one Phase line from the plan.
  if [[ -f "$change_dir/tasks.md" ]]; then
    if ! grep -qE 'Phase 1:' "$change_dir/tasks.md"; then
      echo "tasks.md should reference 'Phase 1:' from the plan"
      fail=1
    fi
  fi

  # proposal.md should mention the capability under New Capabilities.
  if [[ -f "$change_dir/proposal.md" ]]; then
    if ! grep -q "feature-d-unbuilt" "$change_dir/proposal.md"; then
      echo "proposal.md should mention 'feature-d-unbuilt'"
      fail=1
    fi
  fi

  # The plain openspec/specs/<cap>/ landing must NOT exist for change candidates.
  if [[ -e "$dir/openspec/specs/feature-d-unbuilt" ]]; then
    echo "openspec/specs/feature-d-unbuilt should not exist for change candidates"
    fail=1
  fi

  # The translated audit config must NOT reference change candidates in its
  # specs arrays (those caps live at openspec/changes/, not openspec/specs/).
  if [[ -f "$dir/openspec/.audit-config.json" ]]; then
    local has_ref
    has_ref=$(jq -r '.modules[].specs[]' "$dir/openspec/.audit-config.json" | grep -cx "feature-d-unbuilt" || true)
    if [[ "$has_ref" != "0" ]]; then
      echo "openspec/.audit-config.json modules reference 'feature-d-unbuilt' but that capability is at openspec/changes/, not openspec/specs/"
      fail=1
    fi
  fi

  # Source archive should have the change-specific banner.
  local archive="$dir/.workflow/legacy-specs/feature-d-unbuilt.md"
  if [[ -f "$archive" ]]; then
    if ! head -n 5 "$archive" | grep -qF "in-flight OpenSpec change"; then
      echo "$archive: expected change-specific banner; first 5 lines:"
      head -n 5 "$archive"
      fail=1
    fi
  else
    echo "missing archive at $archive"
    fail=1
  fi

  # OpenSpec validation should accept the change.
  if ! (cd "$dir" && OPENSPEC_TELEMETRY=0 openspec validate v1.0-feature-d-unbuilt --strict >/dev/null 2>&1); then
    echo "openspec validate v1.0-feature-d-unbuilt --strict failed"
    fail=1
  fi
...
assert_change_flag_with_missing_plan_fails() {
  local dir
  dir="$(setup_fixture change-missing-plan)"

  local out rc
  out=$(run_migrate "$dir" run --auto-accept --change ghost 2>&1)
  rc=$?
  if [[ $rc -eq 0 ]]; then
    echo "expected non-zero exit when --change refers to a spec with no matching plan"
    echo "output: $out"
    return 1
  fi
  if ! grep -qE "no matching plan|ghost" <<<"$out"; then
    echo "expected error mentioning the missing plan / ghost spec; got:"
    echo "$out"
    return 1
  fi
  # Inventory should NOT exist because we fail before writing it.
  if [[ -f "$dir/migration-inventory.json" ]]; then
    echo "expected migration-inventory.json to NOT exist after fail-fast"
    return 1
  fi
}
...
suite audit-config
if ! skip_if_filtered; then
  test_case "Phase 4 translates legacy <spec-dir>/.audit-config.json to openspec/.audit-config.json" assert_audit_config_translated
  test_case "Phase 4 is a no-op when no <spec-dir>/.audit-config.json exists" assert_audit_config_missing_is_noop
fi

suite change-candidate
if ! skip_if_filtered; then
  test_case "--change flag routes spec to openspec/changes/<name>/ with proposal+tasks+delta" assert_change_candidate_creates_change_folder
  test_case "--change flag with no matching plan fails fast before any work" assert_change_flag_with_missing_plan_fails
fi
