...
- **Complex** (bug fix across multiple components, deletions of non-obvious code, changes where the diff alone does not explain *why*): do 3b thoroughly.
...
- Check which checks failed from the PR status.
- For each failed check:
  - Read the failure logs from the CI system.
...
- Use `mcp__plugin_github_github__pull_request_read` to fetch reviews and comments.
