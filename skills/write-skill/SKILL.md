...
- Use `origin/HEAD`: resolves to whatever the remote default branch is
...
1. **Be concise.** Only add context Claude does not already have.
...
6. **Avoid backticks in prose.** The skill content passes through shell evaluation; inline code fences can break parsing.
