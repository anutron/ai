...
- A requirements/spec doc per decision — detailed enough to hand to an implementer without further input
- Tickets (GitHub issues, Jira, Linear, or whatever tracker the project uses)
- A QA checklist per section
- A slide deck or presentation outline
...
6. Create any artifact templates needed (e.g., a spec/requirements template, a QA template)
...
5. **Offer a design handoff** -- if the interview surfaced problems, gaps, or opportunities that could lead to building something, ask:

   > "We've covered the problem space. Want to move into designing a solution? I can hand off to `/brainstorm` (or whatever your design skill is called) with everything we've discussed."

   If the user says yes, invoke that skill with a summary reference to the interview artifacts.
...
- **Do not resist indefinitely.** If the user pushes back ("no really, I've told you everything, let's go"), respect that and offer the design handoff. The guard is a nudge, not a gate.
